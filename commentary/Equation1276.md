This law implies that left and right multiplications are surjective, that the left cubing map `B: x ↦ x◇(x◇x)` is surjective and the right cubing map `C: x ↦ (x◇x)◇x` is injective, with `B(C(x)) = x`.
