This law is equivalent to stating that all products `x ◇ (y ◇ x)` and all products of the form `(_ ◇ _) ◇ _` are equal to the same element `∞` that is a left and right zero (`∞◇x = x◇∞ = x`).
