...
\begin{proposition}[Asterix implies Oberlix for finite magmas]\label{asterix-oberlix-finite}\uses{eq65,eq1491}  Any finite magma obeying the Asterix law (\Cref{eq65}) also is left-cancellative and obeys the Obelix law (\Cref{eq1491}).
\end{proposition}

\begin{proof}  From \Cref{eq95} we see the map $z \mapsto y \op z$ is surjective, hence injective on a finite magma; thus the magma is left-cancellative.  Replacing $x$ by $y \op x$ in this law, we see that
...
We remark that a very similar argument shows that a finite magma that obeys the Oberlix law has $z \mapsto y \op z$ injective, hence surjective, and then obeys the Asterix law.
