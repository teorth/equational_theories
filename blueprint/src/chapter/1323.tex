...
\begin{lemma}[Greedy extension]\label{greedy-1323}\uses{partial-1323-sound}\leanok\lean{Eq1323.Greedy.extend}  If $\op$ is defined by a partial solution, and $(x,a) \op (y,b)$ is undefined for some $x,y \in \Q^\times$ and distinct $a,b \in G \backslash \{0\}$, then it is possible to extend the partial solution so that $(x,a) \op (y,b)$ is now defined.
\end{lemma}
...
\begin{corollary}[Iterated greedy extension]\label{greedy-iterate}\leanok\lean{Eq1323.}  Every partial solution can be extended to a complete solution that obeys \Cref{lr} and \Cref{lr-simp}, and hence 1323.
\end{corollary}

\begin{proof}\uses{greedy-1323, build-magma, partial-1323-sound} Apply the usual greedy algorithm.
\end{proof}

\begin{corollary}[1323 does not imply 2744]\label{1323-refute-2744}\leanok\lean{Eq1323.Equation1323_not_implies_Equation2744.Greedy.exists_complete_function}  There exists a 1323 magma which does not obey the 2744 equation $R_y L_{Sy} L_y x = x$.
\end{corollary}
