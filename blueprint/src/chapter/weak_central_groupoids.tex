...
\begin{proof}
  It suffices to prove that \eqref{1485} implies \eqref{2162}. Write $w = y \op z$, then from \eqref{1485} we can write $z = z_1 \op z_2$ with $z_1 = z \op z$ and $z_2 = z \op (z \op z)$, and then by \eqref{1485}
...
\begin{proof}\uses{graph-dual}
  If $a \to b \to c$ is good then $b = a \op c$; if $c \to d \to e$ is good then $d = c \op e$; and if $e \to a$ then $a = e \op z$ for some $z$ by definition. By \eqref{2162} we then have
...
\begin{theorem}[1485 does not imply 1483]\label{1485-refutes}\uses{eq1485}\leanok\lean{Refutation_1485.not_3457,Refutation_1485.not_3511,Refutation_1485.not_2087_2124}
  \Cref{eq1485} does not imply any of the following laws:
  \begin{itemize}
...
Occasionally, an equational law is preserved under a ``twist'' operation in which one replaces the magma operation $x \op y$ by $x \op' y := Tx \op Uy$ for some automorphisms $T,U$ of the magma $G$ that obey additional relations.  In the case of the weak central groupoid law \eqref{1485}, we see that
$$
