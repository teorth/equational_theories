...
infinite magma satisfies $P$ without also satisfying $Q$. Examples are given in \Cref{infinite-model-chapter}.
...
(\Equareffull{65}), reviewed in \Cref{asterix-section}.

\section{Translation-invariant magmas}

A \emph{translation-invariant magma} is a magma whose carrier $G$ is an abelian group $(G,+,-,0)$, and whose magma operation takes the form
$$ y \op x = x + f(x-y)$$
for some function $f: G \to G$.  Thus the translations on $G$ become magma isomorphisms.

\begin{example}
  A magma $G$ satisfying the left (\Equaref{4}) or right (\Equaref{5}) absorption laws is translation-invariant.
...
where $f^2 = f \circ f$, so the Asterix equation (\Equaref{65}) for such magmas simplifies to the univariant functional equation
...
$$ E_0 \subseteq E_1 \subseteq E_2 \subseteq \Z$$ together with a function $f: E_1 \to E_2$ with the following properties:
\begin{description}
...
\begin{itemize}
  \item $E_i \subseteq E'_i$ for $i=0,1,2$.
...
\begin{corollary}[Asterix does not imply Obelix]\label{asterix-obelix}\uses{eq65,eq1491}  There exists a magma satisfying the Asterix law (\Equaref{65}) with carrier $\Z$ such that the left-multiplication maps $L_y: x \mapsto y \op x$ are not injective for any $y \in \Z$.  In particular, it does not satisfy the Obelix law (\Equareffull{1491}).
\end{corollary}
...
\begin{proposition}[Asterix implies Obelix for finite magmas]\label{asterix-obelix-finite}\uses{eq65,eq1491}  Any finite magma satisfying the Asterix law (\Equaref{65}) also is left-cancellative and satisfies the Obelix law (\Equaref{1491}).
\end{proposition}

\begin{proof}  From \Equaref{65} we see the map $z \mapsto y \op z$ is surjective, hence injective on a finite magma; thus the magma is left-cancellative.  Replacing $x$ by $y \op x$ in this law, we see that
$$ y \op x = y \op ((y\op x) \op (y \op (y \op x)));$$
using injectivity, we conclude
$$ x = (y\op x) \op (y \op (y \op x))$$
which is \Equaref{1491}.
\end{proof}
...
\section{Obelix}
Obelix magmas are those satisfying equation 1491, \Equaref{1491}:
$$x = (y \op x) \op (y \op (y \op x))$$
...
In \Cref{asterix-section} a magma satisfying one law and refuting another was obtained by using the
...
Such an operation is then associated (via a one-to-one correspondence) to a partially defined operation $\op : S \to G$ for some $S \subseteq G \times G$, with $R(x,y,z)$ holding if and only if $x \op y$ is well-defined (i.e., $(x,y) \in S$) and equal to $z$.  By abuse of notation, we shall also refer to $R$ as a partially defined magma operation.  Genuine magmas then correspond to the special case where $S = G \times G$, that is to say $x \op y$ is well-defined for all $x,y \in G$.
...
Unfortunately, most theories are not greedily extensible without further modification.  Consider for instance the theory $\Gamma$ consisting of (VLT) and the Asterix law (65).  Given $a,b,c$ and a finitely supported $R$ satisfying $\Gamma$ as well as (novel-1), (novel-2), (undefined), we would like to construct a finitely supported  $R'$ satisfying $\Gamma$, (extend), (define).  The naive guess would just be to take the minimal construction
...
Using this, one can for instance find a magma satisfying \Equaref{65} that fails the left-cancellative property
...
\end{equation}
simply by starting with a partial solution, say on the natural numbers in which (e.g.) $R(1,2,0)$, $R(1,3,0)$ are the only situations in which $R$ holds.  One can easily verify that this satisfies (VLT) and (65), (65'), but not \Cref{left-inject}, and so any magma constructed by the above greedy construction will not be left-cancellative.  Since the Obelix law \Equaref{1491} forces left-cancellativity, this gives an alternate proof of \Cref{asterix-obelix}.
...
The implication between the laws \Equareffull{1648} and \Equareffull{206} can be refuted by a translation-invariant
...
so the magma does not satisfy \Equaref{206}. We can conclude
...
\begin{theorem}[1648 does not imply 206]\label{non_imp_1648_206_thm}\uses{eq1648,eq206,models-def}
  ...
  There exists a magma which satisfies \Equaref{1648} and not \Equaref{206}.
\end{theorem}
...
A model refuting the implication between the laws \Equareffull{1659} and \Equareffull{4315} can be constructed using
the modified base model technique. Start with the infinite model $(\mathbb{Z},\op)$ of \Equaref{1659} given by the following operation:
$$ x\op y = \begin{cases} x+1 & \text{if } x,y \text{ have the same parity} \\ x -1 & \text{otherwise} \end{cases} $$
A case analysis shows that the magma $(\mathbb{Z},\op)$  satisfies both \Equaref{1659} and \Equaref{4315}.
...
and checking that $(\mathbb{N}, \op' )$ satisfies \Equaref{1659} but refutes \Equaref{4315}.
...
consequent in the modified model. In this specific case, the consequent, \Equaref{4315}, can be written as
...
a counterexample to \Equaref{4315} sets $0 \op'' 1 = 0$ and $x \op'' y = x \op y$ for any $x \neq 0, y \neq 1$.

Unfortunately, the resulting $(M,\op'')$ would not then satisfy \Equaref{1659}. For any $z \in M$, one would have
...
by considering what happens if $x \neq 0$ and $y \neq 1$, we see that all new counterexamples to \Equaref{4315}
...
operation is nonnegative whenever the operands are nonnegative. This finishes the construction without generating any further violations of \Equaref{1659} and proves
...
\begin{theorem}[1659 does not imply 4315]\label{non_imp_1659_4315_thm}\uses{eq1659,eq4315,models-def}
  ...
  There exists a magma which satisfies \Equaref{1659} and not \Equaref{4315}.
\end{theorem}
...
Now we consider the Dupont equation, \Equareffull{63}, which can be treated by a greedy translation invariant construction that is more complicated than the one considered for the Asterix law in \Cref{asterix-section}.
...
We now extend this greedily to $\Z^3 \times \Z$ as follows.  Define a \emph{partial solution} $(E_1,E_2,f)$ to be a pair $\Z^3 \subseteq E_1 \subseteq E_2 \subseteq \Z^3 \times \Z$, and a function $f: E_2 \to E_1$ satisfying the following axioms.
\begin{description}
...
\end{description}
Thus for instance one can take $(\Z^3,\Z^3,T)$ as a partial solution.  We say that a partial solution $(E'_1, E'_2, f')$ is an \emph{extension} of $(E_1,E_2,f)$ if $E_1 \subseteq E'_1$, $E'_2 \subseteq E_2$, and $f'$ agrees with $f$ on $E_2$.
...
Iterating these lemmas in the usual fashion, we can conclude that any partial solution can be completed to a global model of \Equaref{63}.
...
\begin{proof}  We divide into cases.
  ...
  {\bf Case 2:} $h_0 \in E$ but $f(h_0) \not \in E$.  This is the main case.  Let $H$ be the set of all $h \in E$ such that $f(h) = f(h_0)$; this is a finite set containing $h_0$.  Let $H' \subseteq H$ be the set of all $h' \in H$ such that $-h' \in E$.  We make the following observations:
  \begin{itemize}
...
%There exists a magma that satisfies equation 3475,
%$$ x \op x = y \op ((x \op y) \op y),$$
%but does not satisfy equation 3659,
%$$x \op x = (x \op x) \op (x \op x)$$
%\end{theorem}
%
...
\begin{proof} A first attempt would be the operation $i \op j := j+1$ on $\Z/3\Z$; this satisfies 1437, but unfortunately also satisfies 4269.  However, an "extension" of this operation will work. Namely, we take the carrier $G = \N \times \Z/3\Z$ and define $(a,i) \op (b,j)$ to equal
  \begin{itemize}
