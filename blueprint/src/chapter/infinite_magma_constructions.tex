...
\item (Law 3) If $Sx$ is defined, then $(Sx \diamond x) \diamond x$ is defined.
\end{itemize}
...
Case 1.  $x=c$ or $x \op' y = c$.  Not possible since no left multiplication by $c$ is defined.
Case 2.  $y=c$.  This would force $x = Sb$ and $d = Sb$, but this is not possible since $d$ is different from $Sb$.
Case 3: $(x \op' y) \op' y = c$. This forces $x=d$ and $y=b$, and the claim follows from construction.
Case 4: None of the terms in $(x \op' y) \op' y$ are equal to $c$.  Then the claim follows from Law 1 applied to $\op$.
\end{proof}
