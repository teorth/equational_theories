...
(\Cref{eq65}), so we will begin by reviewing that analysis.
...
this counterexample. But doing that just moves the issue one level higher: one then
$$ (0 \op''' 3) \op''' ((3 \op''' 3) \op''' z) = 0 \op''' (4 \op''' z)$$ which equals $0 \op''' 3 = 0$ as desired for even $z$, but equals $0 \op''' 5 = -1 \neq 0$ for odd $z$.

One sees that the infinitely many counterexamples arising from the iterated redefinition can be eliminated
...
\begin{theorem}[1659 does not imply 4315]\label{non_imp_1659_4315_thm}\uses{eq16459,eq4315,models-def}
  \lean{Equation1659_not_implies_Equation4315}\leanok
