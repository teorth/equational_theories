...
\begin{corollary}[Asterix does not imply Obelix]\label{asterix-obelix}\uses{eq65,eq1491}  There exists a magma obeying the Asterix law (\Equaref{65}) with carrier $\Z$ such that the left-multiplication maps $L_y: x \mapsto y \op x$ are not injective for any $y \in \Z$.  In particular, it does not obey the Obelix law (\Equareffull{1491}).
\end{corollary}
...
\begin{proposition}[Asterix implies Obelix for finite magmas]\label{asterix-obelix-finite}\uses{eq65,eq1491}  Any finite magma obeying the Asterix law (\Equaref{65}) also is left-cancellative and obeys the Obelix law (\Equaref{1491}).
\end{proposition}
...
\section{Obelix}
Obelix magmas are those obeying equation 1491, \Equaref{1491}:
$$x = (y \op x) \op (y \op (y \op x))$$
...
Using this, one can for instance find a magma obeying \Equaref{65} that fails the left-cancellative property
...
so the magma does not obey \Equaref{206}. We can conclude
...
Unfortunately, the resulting $(M,\op'')$ would not then obey \Equaref{1659}. For any $z \in M$, one would have
