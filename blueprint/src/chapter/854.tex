...
Now we define the operation $\to$ on $M_X$ by the following rule: for $x,y \in M_X$, we have $x \to y$ if and only if one of the following holds:
\begin{itemize}
\item (i) $x = y_R$.
...
\end{itemize}
Here we adopt the convention that a claim can only hold if all terms in it are well-defined, fr instance claim (ii) can only hold if $x_{RL}$ is well-defined.  If we define the ``max-length'' of a claim $x \to y$ to be the maximum of the length of $x$ and the length of $y$, we see that the verification of a claim $x \to y$ only involves claims $x' \to y'$ of strictly smaller max-length.  Thus this definition is well-defined.  We also see that if $x \to y$, then exactly one of
\begin{equation}\label{one}
  x=y_R, x=y_{RL}, y = x_R, y=x_{RL}
\end{equation}
hold; in particular, we cannot have $x \to x$ for any $x$. From (v) we also see that
