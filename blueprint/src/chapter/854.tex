...
In this chapter we study magmas that satisfy \Equaref{854}, thus
...
\end{itemize}
If (i) holds, then we have $x \op y = x$ (Equation 4) in $M_X/\sim$, hence in every magma satisfying \Equaref{854}.  But we have examples of such magmas in which this fails.

Similarly, if (iii) holds, then $y \op (x \op y) = y$ (Equation 10) holds in $M_X/\sim$, hence in every magma satisfying \Equaref{854}.  But we have examples of such magmas in which this fails.
...
Introduce the notation $y \leq x$ for $x \op y \to x$.
...
\begin{corollary}[854 does not not imply 413]\leanok\lean{Refutation_854.not_413_1045}\label{854-413}  There is an 854 magma which does not satisfy the 413 law $x = x \op (x \op (x \op (y \op x)))$.
\end{corollary}
...
\begin{corollary}[854 does not not imply 1045]\leanok\lean{Refutation_854.not_413_1045}\label{854-1045}  There is an 854 magma which does not satisfy the 1045 law $x = x \op ((y \op (y \op x)) \op x)$.
\end{corollary}
