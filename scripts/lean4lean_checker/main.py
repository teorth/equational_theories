...
class Lean4LeanChecker:
    ...
    def try_update_l4l_toolchain(self):
        try:
            ...
            if self.checkToolchainMatch():
                ...
                print(
                    "An attempt to update lean4lean to a matching toolchain will now begin"
                )
                ...
                if val == 0:
                    ...
                    print(
                        "There is a toolchain mismatch and lake cannot update lean4lean"
                    )

        except Exception as e:
            print(
                f"Failed to upgrade lean4lean.\n Cause: {e.args}\nCancelling lean4lean check and exiting"
            )
            sys.exit(0)
    ...
    def remove_old_l4l(self):
        try:
            ...
            print(
                "Failed to remove old copies of lean4lean. The checker will not be run"
            )
            sys.exit(0)
    ...
    def build_l4l(self):
        try:
            ...
            build_ret = subprocess.run(
                [self.lakePath.as_posix(), "build", "lean4lean"]
            ).returncode
            if build_ret == 0:
                print("Lean4lean was successfully built\n")
            else:
                print(
                    "Lean4lean build was not successful. Lean4lean check will not be run. Exiting gracefully"
                )
