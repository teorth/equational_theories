...
if __name__ == "__main__":
    ...
    with open(outcomes_json_path, "w") as outcomes_file:
        ...
        outcomes_result = subprocess.run(
            outcomes_command, text=True, check=True, stdout=outcomes_file
        )

    print("generating image...")
    generate_image_command = [
        "./scripts/outcomes_to_image.py",
        outcomes_json_path,
        "--out",
        outcomes_image_path,
    ]
    ...
    outfile.write(
        f"The implication graph is **{ratio:.4%}** complete if we include conjectures.\n\n"
    )
