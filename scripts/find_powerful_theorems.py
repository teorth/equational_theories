...
def main():
    parser = argparse.ArgumentParser(
        description="Find the most useful implications in a matrix."
    )
    parser.add_argument(
        "--topk", type=int, default=1, help="Return the k best theorems to prove"
    )
    parser.add_argument(
        "--one-per-equiv",
        action="store_true",
        help="Return only one per equivalence class",
    )
    parser.add_argument(
        "--load-cache", type=str, help="Load output from the cache file"
    )
    parser.add_argument(
