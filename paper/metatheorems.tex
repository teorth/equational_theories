...
\end{equation}
for some word $f(\x,\y,\z)$, that are also known to imply the right-idempotent law \eqref{eq378}.  An illustrative example is the law $\Eq{854}$ depicted in Figure \ref{fig:854}. Other examples are listed in \Cref{fig:854-like}.
...
\begin{proposition}[$\Eq{854}$ does not imply $\Eq{3316}$]\label{854-3316} Equation $\Eq{854}$ does not imply $\Eq{3316}$ .
\end{proposition}
