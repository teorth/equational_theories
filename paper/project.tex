...
When the number of contributors grows beyond the single digits, it becomes increasingly difficult to ensure that robustness of results. An interactive theorem prover (hereon referred to as ITPs) then acts as a common referee, rigorously checking proofs coded into it by the contributors. While kernels of LCF style theorem provers can be relied on to check proofs of theorems presented to them, it is important to recall the limitations of any computerised proof checking mechanism. We list some of these below:
\begin{itemize}
    \item However well checked and small, a kernel could still contain fatal errors that falsely certifies incorrect proofs.
    \item
\end{itemize}
\begin{itemize}
