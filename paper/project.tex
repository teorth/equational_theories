...
\begin{itemize}
    \item An ITPs proof-checking kernel could still contain fatal errors that result in incorrect proofs being accepted. These are rare, but not unheard of \TODO{Citation needed}. ITPs are however carefully designed, implemented, and reviewed to mitigate this and the kernel is often kept small. For instance, the Lean ITP has a kernel implemented in under 10000 lines of code. \TODO{cite something here}
    \item An ITPs kernel checks whether a proof certificate constitutes a proof of a given theorem statement. It is important to remember that theorem statements are specified in a computer language which assigns a very specific meaning to eacb subexpression. They are in turn built on top of other definitions, that could could have unexpected meanings that diverge in important ways from conventional pen and paper notation. A classic example is the definition of natural number subtraction in many type theoretic theorem provers, where a conscious design choice is made to assign the value 0 when the second argument is larger than the first. Thus it is important to check all the definitions and theorem statements of an ITP proof development. Further, developing standardised syntax for writing complex definitions and theorems, with proven semantics, can greatly ease the process of checking definitions and proofs.
\end{itemize}
\begin{itemize}
