...
**Already handled:** the 23 items struck through below are done in the manuscript — all of Referee #2’s typos except R2.T10, both citation items (R2.1, R2.2), and, from Referee #1, R1.11–R1.14, R1.19–R1.21, R1.24 and R1.M1, covered by the new introduction paragraphs on why Lean was chosen, on project logistics, on the intermediate level of automation, and on modularity. **31 items remain unclaimed.**
...
- ~~**R1.11**~~ **[DONE]** — Add a detailed comparison with other large collaborative projects; the referee finds the paper's stated aim unsupported without it.
- ~~**R1.12**~~ **[DONE]** — Discuss the Liquid Tensor Experiment: it predates PFR, was arguably the first blueprint-centred project, and was more centralized and hierarchical. Without this, the referee says the honest framing is that ETP experimentally confirmed the LTE model.
- ~~**R1.13**~~ **[DONE]** — Treat BB(5) more accurately (coordinated on Discord and a wiki; largely without proof assistants, formalized after the fact) and discuss how those coordination choices compare with ours.
- ~~**R1.14**~~ **[DONE]** — Discuss earlier large collaborations — Flyspeck, the odd order theorem, and more loosely the AFP and math-comp — and what distinguishes a large *mathematical* collaboration from large-scale software/proof-assistant development.
...
- ~~**R1.19**~~ **[DONE]** — Give more prominence to the observation that strong automation early in a project kills insights — currently buried in §14.1, and in the referee's view one of the most valuable remarks in the paper.

*Discussion of the choice of Lean 4*

- ~~**R1.20**~~ **[DONE]** — Answer the *Virtues of a Formalization Project* question: did the choice of proof assistant or logical foundation crucially influence the formalization process? Referee identifies three factors: ATP capabilities, native computation, mathematical libraries.
- ~~**R1.21**~~ **[DONE]** — Compare with Isabelle, where `sledgehammer` calls CVC4/Vampire natively and proof reconstruction is supported out of the box, against the custom reconstruction architecture we had to build.
...
- ~~**R1.24**~~ **[DONE]** — Address the referee's impression that Mathlib — usually the decisive reason to pick Lean — did not actually matter for this project.
