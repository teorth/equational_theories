...
\noindent
\textbf{Status.} Items whose response below begins ``Done'' have been implemented in the
manuscript and verified against a clean build. \textbf{R2.T4} and \textbf{R2.T6} are
...
\response{Done. The phrase ``the proof assistant language \emph{Lean}'' has been replaced by
``the language of the proof assistant \emph{Lean}'' at its three occurrences in the
introduction and in \S2, and by ``the formal proof assistant \emph{Lean}'' in the
abstract (see \textbf{R2.T1}). The one remaining occurrence, ``the proof assistant
language \emph{Coq}'', has been left alone for now, since \textbf{R2.T4} rewrites the
same clause.}
...
\response{Done. \emph{Lean} is now cited at its first mention in the introduction, rather
than only at the later discussion in \S4.7; \emph{duper} and \emph{egg} are now cited at
their first mention in \S2; and the \emph{Lean blueprint} tool is now cited where it is
first named, in addition to the citation already attached to the earlier, generic mention
of a blueprint. All four already had bibliography entries; no new ones were needed for
this item.}
...
\response{Done. Bibliography entries have been added for \emph{Mathlib}, \emph{SageMath},
\emph{GAP} and the \emph{SmallGrp} small groups library, in each case following the
citation form recommended by the project itself at the address the referee kindly
supplied, and each is now cited at its first occurrence in the text. \emph{GAP} and
\emph{SmallGrp} are cited together at the sole mention of the small groups library in
\S7. One caveat for our own attention: the \emph{GAP}, \emph{SmallGrp} and
\emph{SageMath} entries currently record the versions that those projects recommend
citing at the time of writing, and should be replaced by the versions actually used
during the ETP.}
...
\textbf{R1.M1}.)
\response{Done. The abstract now reads ``all validated by the formal proof assistant
\emph{Lean}'', the first of the referee's two suggested formulations. See also
\textbf{R1.M1}.}

\item (p.~3, line~4) ``In this entailment pre-ordering'' \(\to\) ``In this entailment
pre-order''.
\response{Done. Changed to ``In this entailment pre-order''. This also removes an internal
inconsistency, as the preceding paragraph already used ``pre-order''.}
...
\response{Done. The clause now reads ``contributions had to be written in the language of the
proof assistant \emph{Lean}'', which also disposes of \textbf{R1.M1} at this
occurrence.}
...
\response{Done, and we thank the referee for catching the ambiguity: their reading is correct.
The sentence now states that the finite implication held for an additional 820 pairs, and
that for a further 2 pairs --- a single pair up to duality --- the status of the finite
implication remains unknown, with a cross-reference to the section on implications for
finite magmas, where that open problem is stated.}
...
\response{Done. Corrected to ``two words in a free magma''. The referee is right:
\texttt{MagmaLaw} in the Lean sources is a structure whose \texttt{lhs} and \texttt{rhs}
fields both have type \texttt{FreeMagma}. The other occurrence of ``free group'' in the
same section refers to Mathlib's free group, is correct, and has been left as it
stands.}

\item (p.~12, line~11) ``LeanBlueprint'' is used, but p.~13, line~22 and thereafter use
``Lean blueprint''. Use one form consistently.
\response{Done. The single occurrence of ``LeanBlueprint'' has been changed to ``Lean
blueprint'', which is the form used at every other mention in the paper.}
...
\response{Done. This was a typo rather than a second tool: it is \emph{lean4checker} in both
places, and has been corrected. The two passages describe the same thing --- Lean's
environment replay tool, named alongside \emph{lean4lean} --- which is what makes the
slip identifiable.}
...
\response{Done. The sentence now refers to ``versions of the greedy algorithm of
Theorem~5.12'', cross-referenced to the abstract greedy algorithm, which is indeed the
result intended.}
...
\response{Done. The sentence now reads ``\dots\ comes from the free magma
$\Magma_{X,\Gamma}$ of a theory, defined in Section~2''.}

\item (p.~38, line~18) ``Clearly the term being rewritten is in \dots'' should be
``Clearly, if the term being rewritten is in \dots''.
\response{Done. Corrected to ``Clearly, if the term being rewritten is in \dots''.}
