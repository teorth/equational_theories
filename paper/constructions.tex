...
By performing brute force over all magmas up to order $4$, a total of $13,632,566$ implications ($61.9\%$ of all implications) can be resolved with $524$ distinct magmas. Of these implications, $13,345,053$ are resolved with $3 \times 3$ magmas with the remaining $415,293$ requiring $4 \times 4$ magmas. Performing this search take 165 CPU-hours.
...
\begin{proposition}[1110 does not imply 1629]\label{1110-1629} The law $\x \formaleq \y \op ((\y \op (\x \op \x)) \op \y)$ \eqref{eq1110} does not imply $\x \formaleq (\x \op \x) \formaleq ((\x \op \x) \op \x)$ \eqref{eq1629}, even for finite magmas.
\end{proposition}
