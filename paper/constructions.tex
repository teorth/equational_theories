...
\begin{proof}  (Sketch) Using the linear ansatz, we find that \Cref{eq1110} has a model $M$ with carrier $\F_5$ (the finite field $\Z/5\Z$) with operation $x \op y = 3x-y$.  We then apply the ansatz \eqref{xyst} with $G=M$.  One then finds that this operation obeys \eqref{eq1110} if $f \colon \F_5 \times \F_5 \to \F_5$ obeys the cocycle equation
...
Now we give an example of how this method (ii) can be combined with method (i).
...
\begin{proof}  There are two simple models for \eqref{eq1659}: the model $G = \Z/2\Z$ with $x \op y = x+1$, and the model $M = \Z$ with model $x \op y = x$.  Using the ansatz \eqref{xyst}, one can soon discover that one obtains a magma operation $\op: (G \times M) \times (G \times M) \to G \times M$ with $f(0,0)=f(1,0)=0$, $f(0,1)=-1$, and $f(1,1)=1$.  This model still obeys \eqref{eq4315}. However we can create a modification $\op'$ of $\op$ as follows.  We will seek to violate \eqref{eq4315} at $x = (0,0)$, $y = (0,0)$, $z = (1,0)$, thus we want
...
\note{TODO: Talk about 854 constructions, squares-first constructions}
