...
\begin{proof}  (Sketch) Using the linear ansatz, we find that \eqref{eq1110} has a model $M$ with carrier $\F_5$ (the finite field $\Z/5\Z$) with operation $x \op y = 3x-y$.  We then apply the ansatz \eqref{xyst} with $G=M$.  One then finds that this operation obeys \eqref{eq1110} if $f \colon \F_5 \times \F_5 \to \F_5$ obeys the cocycle equation
...
The law $\x \formaleq \x \op ((\y \op \z) \op (\x \op \z))$ \eqref{eq854}
