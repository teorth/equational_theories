...
/-- Extend a partial solution at an element x in its domain, so that x satisfies the functional equation. -/
...
/-- Extend makes sure that x satisfies the functional equation -/
...
/-- Given f, (possibly) extend it to ensure that x is in the domain, image, and satisfies the functional
  equation. This is done by possibly using `add` to include x, and then `extend`. -/
def close (f : PartialSolution) (x : A) : PartialSolution :=
  if hx : x ∈ f.Dom then
    --If x is in the domain, just make sure that it satisfies the functional equation with `extend`.
    f.extend hx
...
/-- The resulting magma satisfies the Obelix rule. -/
