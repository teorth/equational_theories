...
theorem A_not_FG : ¬(AddGroup.FG A) :=
  fun _ ↦ DFinsupp.infinite_of_left.not_finite <| AddCommGroup.finite_of_fg_torsion A (fun _ ↦ by
    rw [isOfFinAddOrder_iff_nsmul_eq_zero]
    use 3
    constructor
    · simp
    · ext
      simp [show (3:Fin 3) = 0 by rfl]
  )
...
def freshGenerator (f : PartialSolution) (x : A := 0) : A := by
  exact fun₀ | 17 => 1
  -- have : ∃ x, x ∉ AddSubgroup.closure (f.Dom ∪ f.Im ∪ {x} : Set A) := by
  --   sorry
  -- have := Denumerable.ofNat A (Nat.find (show ∃ n, (Denumerable.ofNat A n : A) ∉ AddSubgroup.closure (f.Dom ∪ f.Im ∪ {x} : Set A)by
  --   obtain ⟨x,hx⟩ := this
  --   sorry
  --   ))
...
theorem freshGenerator_not_dom (f : PartialSolution) : f.freshGenerator ∉ f.Dom := by
...
theorem freshGenerator_not_img (f : PartialSolution) : f.freshGenerator ∉ f.f '' f.Dom := by
  sorry

theorem freshGenerator_not_eq_dom (f : PartialSolution) {x : A} (h : x ∈ f.Dom) : x + x ≠ f.freshGenerator := by
  sorry

theorem freshGenerator_not_eq_img (f : PartialSolution) {x : A} (h : x ∈ f.f '' f.Dom) : x + x ≠ f.freshGenerator := by
  sorry
...
theorem extend_mono (f : PartialSolution) {x : A} (hx : x ∈ f.Dom) :
  ...
  · simp [extend, hf]
    ...
    · have : f.freshGenerator ≠ y + f.f x := by
        sorry --by subgroup
      have : y ≠ f.freshGenerator - f.f x := this.symm ∘ add_eq_of_eq_sub
      simp [hyf, hy, this]
...
theorem extend_dom (f : PartialSolution) {x : A} (hx : x ∈ f.Dom) :
  ...
  · simp [extend, hf]
    ...
    · have : f.freshGenerator ≠ x + f.f x := by
        sorry --by subgroup
      have : x ≠ f.freshGenerator - f.f x := this.symm ∘ add_eq_of_eq_sub
      simp [hxf, this]
...
theorem close_dom (f : PartialSolution) {x : A} :
  ...
  · rw [closeImg]
    apply extend_mono_dom
    exact closeImg.proof_5 f x h₂ _ (closeImg.proof_4 f x h₂ _)
  · rename_i h
    simp only [closeImg]
    apply extend_mono_dom
    let h₃ := close.proof_3 _ _ h
    let h₄ := close.proof_5 f x h _ (extend_mono_dom _ _ _ h₃) (close.proof_4 _ _ h h₃)
    exact closeImg.proof_5 _ _ h₄ _ (closeImg.proof_4 _ _ h₄ (closeImg.proof_3 _ _ h₄))
  · apply extend_mono_dom
    apply add_dom
