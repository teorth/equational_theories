...
/-- The laws a chain passes through, starting at `n`. -/
def Chain.laws (pos : RArray PosFact) (n : Nat) (c : Chain) : List Nat :=
  n :: c.map fun k => (pos.get k).conc

/-- A chain rendered as `Law2 ⟹ Law3 ⟹ Law47`. An empty chain is just `Law2` (reflexivity). -/
def Chain.render (pos : RArray PosFact) (n : Nat) (c : Chain) : String :=
  String.intercalate " ⟹ " ((Chain.laws pos n c).map lawName)
...
def renderResolution (C : Certificate) (pos : RArray PosFact) (neg : RArray NegFact)
  ...
  | .inl c =>
      ...
      s!"  {c.length} step(s): {Chain.render pos n c}"
  | .inr r =>
      s!"{lawName n} ⊭ {lawName m}  (refuted)\n" ++
      s!"  witness #{r.fact} satisfies {lawName r.a} and refutes {lawName r.b}\n" ++
      s!"  {lawName r.a} ⟹ {lawName n}: {Chain.render pos r.a r.up}\n" ++
      s!"  {lawName m} ⟹ {lawName r.b}: {Chain.render pos m r.down}"
