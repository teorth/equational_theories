...
def process_equation_line(line, equation_index):
    ...
    nvar = len(variables)
    allvars = ", ".join("uint64_t " + x for x in "xyzwuv")
    func_code = f"bool fn{equation_index}(uint64_t pk, int inv, uint64_t a, uint64_t b, {allvars}) {{ return MOD(({lhs_c_code}) - ({rhs_c_code}),inv,pk) == 0; }} int nvar_{equation_index} = {nvar};"
    return func_code
...
def generate_c_code(expr, variable_indices):
    if isinstance(expr, Var):
        ...
        if expr.op == "◇":
            ...
            return f"a*({left_code})+b*({right_code})"
        else:
            # Unknown operator
