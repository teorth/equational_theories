...
including this shard -- is indexed from 0. Add 993 to get an index into
`tsccdn`.
-/
...
/-- 784 entries, loaded from `json/sccDn.json`. -/
def tsccdn1 : RArray (List Nat) :=
