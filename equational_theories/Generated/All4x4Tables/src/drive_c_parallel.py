...
def run_batch(batch_number):
    ...
    try:
        ...
        print(
            f"Batch {batch_number + 1}/{TOTAL_BATCHES} completed: tables {start_index} to {end_index}"
        )
        if result.stderr:
            print(
                f"Errors in batch {batch_number + 1}:", result.stderr, file=sys.stderr
            )
...
def main():
    if not os.path.exists(C_PROGRAM_NAME):
        print(
            f"Error: {C_PROGRAM_NAME} not found. Please compile the C program first.",
            file=sys.stderr,
        )
        sys.exit(1)
