...
def E0List : List (A × A) := [(1, x₁), (x₁, x₂), (x₂ * x₁⁻¹,x₁⁻¹), (x₁^(-2 : ℤ), x₁^(-2 : ℤ))]
...
structure PreExtension.OK (E : PreExtension) : Prop where
  ...
  eq1526 {a b c} : b ∈ E a → c ∈ E (b * a⁻¹) → a⁻¹ * x₁⁻¹ ∈ E (c * x₁⁻¹)
...
inductive Next : A → A → Prop
  ...
  | extra {a e f} : Relevant a → e = c * x₁⁻¹ → f = a⁻¹ * x₁⁻¹ →  Next e f -- Nicer for pattern matching
...
theorem next_eq1526 {a b c'} : b ∈ next a → c' ∈ next (b * a⁻¹) → a⁻¹ * x₁⁻¹ ∈ next (c' * x₁⁻¹)
  | .base h1, .base h2 => .base $ ok.eq1526 h1 h2
  ...
  | .new rfl rfl, .extra (.mk h q) he rfl => by
    simp only [mul_right_inj, inv_inj] at he
    exfalso
    exact d_ne_x1 he
...
theorem next_ok : next.OK where
  finite := by
    ...
    (this.finite.map fun a => ((c * x₁⁻¹),(a⁻¹ * x₁⁻¹))))).subset fun (x,y) hx => ?_
...
theorem exists_extension :
  ...
  · let S : Finset A := {x, op x * x⁻¹, 1, op (op x * x⁻¹)*(op 1)⁻¹}
    ...
    exact e.2.func opfinal (eq.symm ▸ e.2.eq1526 opx opopxxinv)
...
theorem f_eq1526 (e₀ : Extension) : (∀ x, f e₀ (f e₀ (f e₀ x * x⁻¹)*(f e₀ 1)⁻¹) = x⁻¹ * (f e₀ 1)⁻¹) :=
...
theorem eq1526 : Equation1526 (GreedyMagma e₀) := by
  intro x y
...
@[equational_result]
theorem not_1223 : ∃ (G : Type) (_ : Magma G), Facts G [1526] [1223] := by
  have ⟨e, he⟩ : ∃ e : Extension, e.1 = fromList
    (E0List ++ [(x₁⁻¹,x₃),(x₁⁻¹*x₃⁻¹,x₄),(x₄*x₃*x₁,x₅),(x₅*x₁⁻¹,x₃)]) :=
    ⟨⟨_, fromList_ok⟩, rfl⟩
  refine ⟨GreedyMagma e, inferInstance, eq1526 e, fun h => ?_⟩
  · have := h (1 : A)
    ...
    fromList_eval he (x₁^(-1) * x₃^(-1)) x₄, fromList_eval he (x₄*x₃*x₁) x₅] at this
...
@[equational_result]
theorem not_2744 : ∃ (G : Type) (_ : Magma G), Facts G [1526] [2744] := by
  have ⟨e, he⟩ : ∃ e : Extension, e.1 = fromList
    (E0List ++ [(x₃, x₄),(x₄*x₁⁻¹,x₅),(x₁⁻¹*x₅⁻¹,x₆)]) :=
    ⟨⟨_, fromList_ok⟩, rfl⟩
  refine ⟨GreedyMagma e, inferInstance, eq1526 e, fun h => ?_⟩
  · have := h x₃ (1 : A)
    repeat rw [Magma.op, instMagma] at this
    ...
    repeat rw [op] at this
    group at this
    rw [fromList_eval he 1 x₁, fromList_eval he x₃ x₄, fromList_eval he (x₄*x₁^(-1)) x₅,
