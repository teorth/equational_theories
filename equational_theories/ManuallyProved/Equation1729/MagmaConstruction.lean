...
abbrev PartialSolution_with_axioms.d₀ (sol: PartialSolution_with_axioms) := E (sol.fresh_generator {sol.x} 0)

abbrev PartialSolution_with_axioms.d₁ (sol: PartialSolution_with_axioms) := E (sol.fresh_generator {sol.x} 1)
    ...
    axiom_P' := by
      ...
      | old x'' y'' z'' hI' hxx' =>
        ...
        | P₁ y''' z''' hI'' hz' =>
          ...
          exfalso
          exact sol.invis_lemma y''' z''' (hy'.2.2 ▸ (sol.I_involved {sol.x} hI').2.2)
        | P₂ y''' z''' hI'' hz' =>
          simp only [ne_eq, Function.Embedding.coeFn_mk, Prod.mk.injEq] at hy'
          simp only [hy'.1, hy'.2.1, hy'.2.2] at hI'' hz'
          exfalso
          exact sol.invis_lemma y''' z''' (hy'.1 ▸ (sol.I_involved {sol.x} hI').1)
      | P₁ y'' z'' hI' hz =>
        simp only [ne_eq, Function.Embedding.coeFn_mk, Prod.mk.injEq] at hy
        simp only [hy.1, hy.2.1, hy.2.2] at hI' hz
        cases data' with
        ...
        | P₂ y''' z''' hI'' hz' =>
          ...
          simp only [hy'.1, hy'.2.1, hy'.2.2] at hI'' hz'
          exfalso
          exact sol.invis_lemma y''' z''' (hy'.1 ▸ (sol.I_involved {sol.x} hI').2.2)
      | P₂ y'' z'' hI' hz =>
        simp only [ne_eq, Function.Embedding.coeFn_mk, Prod.mk.injEq] at hy
        simp only [hy.1, hy.2.1, hy.2.2] at hI' hz
        cases data' with
...
lemma enlarge_S' (sol : PartialSolution) (x:N) : ∃ sol', sol ≤ sol' ∧ x ∈ sol'.Dom_S' := by
  apply WellFoundedLT.induction x
  intro z hz
  by_cases z_one: z = 1
  .
    have hind : ∀ y: N, y < z → y ∈ sol.Dom_S' := by simp [z_one, ← bot_eq_one]
    exact enlarge_S'_induction hind
  .
    have h_z_parent := parent_lt z_one
    obtain ⟨parent_sol, h_parent_sol, h_parent_z_in⟩ := hz (parent z) (h_z_parent)
    have hind : ∀ y: N, y < z → y ∈ parent_sol.Dom_S' := by
      intro y hy
      have le_pred := PredOrder.le_pred_of_lt hy
      exact parent_sol.axiom_S (parent z) y h_parent_z_in le_pred
    obtain ⟨sol', hsol', z_sol'⟩ := enlarge_S'_induction hind
    refine ⟨sol', ?_, z_sol'⟩
    exact Preorder.le_trans sol parent_sol sol' h_parent_sol hsol'
  ...
  set sol' : PartialSolution := {
    ...
    axiom_vii'' := by
      ...
      . simp only [hop1.1, hop1.2, ↓reduceIte, Sum.inr.injEq, Finset.mem_union, Finset.mem_singleton,
        ...
        right
        by_cases hw : w ∈ sol.Dom_L₀'
        . simp only [hw, ↓reduceIte, hxy, and_false, and_self, z', new_L₀', and_true, hx]
        simp only [hw, ↓reduceIte, hxy, and_false, Sum.inr.injEq, new_L₀', z', and_true, hx, true_and]
        apply subset_fill _
