...
- [`FactsSyntax.lean`](equational_theories/FactsSyntax.lean)  Support for assertions that a given magma satisfies one set of laws but fails another.
...
- To assert that a magma `G` satisfies one set `X1, X2, ...` of laws but fails another set `Y1, Y2, ...`, use
...
- The pre-ordering `≤` on laws is given by implication.  That is to say, `LawX ≤ LawY` means that `LawX` implies `LawY`, i.e., every magma that satisfies `LawX`, also satisfies `LawY`.
